...
export class BaseClient implements IClientConnector {
    ...
    public sendDataToRemoteApp(toUID: string, dataForRemote: string): void {
        let fromUID: string = this.uid;
        const data = DataSerializer.buildPacket(PacketType.APP_DATA_FOR_REMOTE_APP, DataSerializer.serializeDataForRemoteApp, fromUID, toUID, dataForRemote);
    }
